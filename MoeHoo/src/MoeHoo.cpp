...
std::pair<uint64_t, FuncPtr> searchRkeyDownloadHook()
{
	...
	do
	{
		...
		if (pmap->module_name.find("wrapper.node") != std::string::npos && pmap->executable && pmap->readable)
		{
			...
			// 需要判断
			uint64_t beforeOffect = SearchRangeAddressInModule(pmap, hexPattern_Before_v, 0x3000001);
			...
			if (beforeOffect <= 0)
			{
				...
				beforeOffect = SearchRangeAddressInModule(pmap, hexPattern_Before_v2, 0x3000001);
